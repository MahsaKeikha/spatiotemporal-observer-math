...
Suppose an observable calibration step produces
...
\qquad F_j=\|PR_{\phi_j}P\|_F,
\qquad S_j=\|PR_{\phi_j}P\|_2.
\]

Define derivative envelopes

\[
L_F(\rho)=\left[2\sum_{k=1}^{N-1}(N-k)k^2\rho^{2(k-1)}\right]^{1/2},
\]

and

\[
L_2(\rho)=2\sum_{k=1}^{N-1}k\rho^{k-1}.
\]
...
\ge \min_j d_j-\frac{qL_2(\rho)h}{2},
\]

\[
\operatorname{tr}(PR_\phi)
\le \max_j d_j+\frac{qL_2(\rho)h}{2},
\]

\[
\|PR_\phi P\|_F
\le \max_j F_j+\frac{L_F(\rho)h}{2},
\]
...
\[
\|PR_\phi P\|_2
\le \max_j S_j+\frac{L_2(\rho)h}{2}.
\]

These bounds may safely be intersected with Proposition 45's global fallbacks. Let the resulting certified values be
...
\[
\varepsilon_{\rm oracle}=\frac{4(F_+\sqrt{u}+S_+u)}{d_-}.
\]
...
\[
\frac{d}{d\phi}(R_\phi)_{ij}=|i-j|\phi^{|i-j|-1},
\]
...
The absolute row sum gives

\[
\|R_\phi'\|_2
\le\|R_\phi'\|_\infty
\le2\sum_{k=1}^{N-1}k\rho^{k-1}
=L_2(\rho).
\]
...
For the normalization, use \(\operatorname{tr}(R_\phi)=N\):
...
Every point in the interval lies within \(h/2\) of some grid point. Applying the Lipschitz inequalities at that nearest point proves the continuum bounds. Intersecting them with the independently valid Proposition 45 fallbacks preserves validity.

Finally, on the calibration event the true \(\phi\) belongs to the reported interval. Proposition 44's Gaussian quadratic-form event is then dominated by the stated worst-case oracle radius. The midpoint-normalization algebra is the same as Proposition 45, and a union bound combines the two failure events. \(\square\)
...
Proposition 46 directly evaluates how this particular \(H\) interacts with the AR(1) family and then rigorously fills the continuum between evaluations.
...
- at \(q=24\), the rank-only lower normalization collapses to `4`, producing a radius about `629.1`, while the design-specific proof retains a lower normalization above `103.6` and radius about `5.833`;
- at \(q=25\), the rank-only lower normalization is negative (`-8.33`) and cannot certify the covariance, while Proposition 46 retains a lower normalization about `99.75` and a finite radius about `5.904`.

The purpose is structural. The failure of the rank-only certificate can be an artifact of throwing away the declared nuisance geometry rather than a failure of covariance identifiability after projection.
...
Proposition 46 makes the AR(1) interval geometry-aware, but it still uses the earlier sphere-net concentration step. Proposition 47 replaces that concentration layer when the projected temporal spectrum is known. Proposition 48 will combine the two ideas.
