...
After nuisance projection and population whitening, the covariance estimator has the distribution

\[
S
=\frac{1}{d}\sum_{i=1}^r \lambda_i g_i g_i^\mathsf T,
\qquad
g_i\stackrel{\mathrm{iid}}\sim\mathcal N(0,I_m),
\]

where \(\lambda_i>0\) are the positive eigenvalues of the projected temporal matrix
...
Thus \(\mathbb E S=I_m\). Define centered self-adjoint summands
...
For a scalar \(a\in[0,1/2)\), the rank-one structure gives

\[
\exp(a g g^\mathsf T)
=I+
\frac{e^{a\|g\|^2}-1}{\|g\|^2}gg^\mathsf T.
\]
...
\[
\Pr\!\left[
\lambda_{\max}\!\left(\sum_i X_i\right)\ge t
\right]
\le
m\exp(-\theta t)
\prod_i c_m^+(\theta\lambda_i).
\]
...
\log(2B_0m/\alpha)
+
\sum_i\log c_m^+(\theta\lambda_i)
}{\theta}.
\]

Any admissible \(\theta\) yields a valid bound. Therefore minimizing over a deterministic finite set of candidate \(\theta\)-values improves tightness without introducing a discretization assumption into the probability statement.
...
\[
\varepsilon_{\mathrm{mat}}
=
\max\{\varepsilon_+,\varepsilon_-\}.
\]
...
That behavior is desirable: a few dominant temporal modes should be recognized as less informative than the same total variance spread over many modes.
...
The strongest comparison occurs at `N=850`:
...
Two rows are especially important. At `phi=0.65`, the previous radius is above one (`1.077`), while Proposition 47 gives `0.459`. At `phi=0.80`, the previous radius is `1.630`, while the new matrix radius remains below one at `0.653`.

At the shorter `N=300` record, the matrix result also changes the usable regime: at `phi=0.65`, the radius improves from `2.165` to `0.832`.
...
The result removes the `9^m` sphere-net factor and its associated leading `4` from the known-temporal-geometry covariance layer. It also proves that the high-correlation “radius above one” problem seen after Proposition 45 was not inherent to Gaussian temporal dependence; a substantial part of it came from the concentration technique.

## Remaining frontier

Proposition 47 currently assumes the projected temporal eigenvalues are known. Propositions 45–46 handle an **estimated AR(1) interval**, but only through norm envelopes. The next step is therefore to combine the two ideas:

> build a uniform matrix-Chernoff covariance radius over the entire calibrated AR(1) interval while retaining the actual nuisance design.

That composition would give an end-to-end observable result with unknown time-varying mean, estimated temporal dependence, design-specific nuisance geometry, and the sharper matrix concentration layer all at once.
