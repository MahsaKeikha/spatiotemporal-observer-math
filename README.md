...
connects the central question, all 43 proved statements, all 29 reproducible
...
[Experiment AC: same-record AR(1) estimation and centered covariance calibration](docs/reproducible_results.md#experiment-ac-same-record-ar1-estimation-and-centered-covariance-calibration)
estimates a shared nonnegative temporal correlation from increment energy and
propagates its confidence interval through mean-centered covariance recovery.
The correlation and covariance calculations deliberately reuse the same
record; their 97.5% joint statement follows from a union bound, not an
independence assumption. All 896 recorded joint events are covered. At
correlation `0.97`, accounting for estimated dependence makes the analytical
radius 1.21 times the known-correlation radius.

[Complete same-record calibration](docs/reproducible_results.md#experiment-ac-same-record-ar1-estimation-and-centered-covariance-calibration) ·
[machine-readable results](docs/estimated_ar1_calibration.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-43-same-record-ar1-calibration-and-covariance-screening)
...
[Experiment AB: mean-centered dependent Gaussian calibration](docs/reproducible_results.md#experiment-ab-mean-centered-dependent-gaussian-calibration)
removes the known-mean restriction from the preceding theorem. For separable
Gaussian dependence, subtracting the sample mean requires the exact
normalization \(d_R=\operatorname{tr}(PR)\), rather than automatically using
\(N-1\). All 896 recorded centered-covariance events are covered, and adding a
large constant mean changes the corrected covariance by at most `1.34e-15`.

[Complete centered calibration](docs/reproducible_results.md#experiment-ab-mean-centered-dependent-gaussian-calibration) ·
[machine-readable results](docs/dependent_centered_gaussian_calibration.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-42-mean-centered-separably-dependent-gaussian-screening)
...
[Experiment AA: dependent Gaussian covariance calibration](docs/reproducible_results.md#experiment-aa-dependent-gaussian-covariance-calibration)
replaces the i.i.d. sample assumption by an exact separable Gaussian covariance
model. As stationary AR(1) correlation increases, the theorem reports the loss
of effective sample size and widens the covariance radius accordingly. All 896
recorded dependence-aware events are covered; the i.i.d. radius covers only 19
of 128 draws at correlation `0.85` and none at `0.93` or `0.97`.

[Complete calibration table](docs/reproducible_results.md#experiment-aa-dependent-gaussian-covariance-calibration) ·
[machine-readable results](docs/dependent_gaussian_calibration.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-41-separably-dependent-gaussian-covariance-screening)

[![Estimated drift and refreshed-reference comparison](docs/calibrated_drift_comparison.png)](docs/reproducible_results.md#experiment-z-estimating-drift-versus-refreshing-the-reference)

[Experiment Z: estimating drift versus refreshing the reference](docs/reproducible_results.md#experiment-z-estimating-drift-versus-refreshing-the-reference)
assigns an explicit confidence budget to a statistically estimated population
drift. Across 640 paired trials, every declared event is covered, but directly
refreshing the reference with the same current calibration data produces a
substantially smaller safe graph throughout the tested range.

[Complete comparison table](docs/reproducible_results.md#experiment-z-estimating-drift-versus-refreshing-the-reference) ·
[machine-readable results](docs/calibrated_drift_comparison.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-40-statistically-calibrated-population-drift-envelope)

[![Pilot-normalized screening under declared covariance drift](docs/drift_robust_relative_calibration.png)](docs/reproducible_results.md#experiment-y-screening-under-declared-population-drift)

[Experiment Y: screening under declared population drift](docs/reproducible_results.md#experiment-y-screening-under-declared-population-drift)
tests the pilot-normalized certificate after a covariance change that preserves
every population information factor and the optimal path. Across 448 draws the
declared events remain covered, while the safe graph becomes complete as the
drift envelope grows. The figure therefore records both validity and the point
at which the bound ceases to prune competitors.

[Complete drift table](docs/reproducible_results.md#experiment-y-screening-under-declared-population-drift) ·
[machine-readable results](docs/drift_robust_relative_calibration.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-39-drift-robust-pilot-normalized-screening)

[![Reusable pilot geometry and adaptive Gaussian screening](docs/cross_fitted_relative_calibration.png)](docs/reproducible_results.md#experiment-x-reusable-pilot-adaptive-screening)

[Experiment X: reusable-pilot adaptive screening](docs/reproducible_results.md#experiment-x-reusable-pilot-adaptive-screening)
uses a high-precision pilot covariance to construct observable candidate-specific
radii for later screening cohorts. Across 1,152 screening draws, the adaptive
radius averaged about half the fixed simultaneous radius and produced a smaller
safe graph in every regime.

[Complete adaptive table](docs/reproducible_results.md#experiment-x-reusable-pilot-adaptive-screening) ·
[machine-readable results](docs/cross_fitted_relative_calibration.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-38-pilot-normalized-adaptive-screening)

[![Absolute and covariance-normalized screening comparison](docs/relative_covariance_calibration.png)](docs/reproducible_results.md#experiment-w-covariance-normalized-screening)

[Experiment W: covariance-normalized screening](docs/reproducible_results.md#experiment-w-covariance-normalized-screening)
tests a relative Wishart certificate on the same 18 regimes and the same paired
trajectory draws. It removes the global eigenvalue-floor penalty from the
theorem and retains exactly the planted five-state/four-edge tube in every
recorded trial.

[Complete paired table](docs/reproducible_results.md#experiment-w-covariance-normalized-screening) ·
[machine-readable results](docs/relative_covariance_calibration.json) ·
[proof](docs/proofs_and_conjectures.md#proposition-37-covariance-normalized-gaussian-screening)
...
| --- | --- |
| [Proved statements](docs/proofs_and_conjectures.md) | 43 propositions |
| [Reproducible studies](docs/reproducible_results.md) | 29 experiments, A through Z and AA through AC |
| [Scientific figures](docs/research_overview.md#visual-record-of-the-results) | 16 committed figures |
| [Claim-level tests](docs/experimental_protocol.md#9-tests-tied-to-scientific-claims) | 130 of 130 passing |
...
and weakening the screen. Proposition 37 resolves that specific artifact with a
population-whitened relative covariance event. On 1,152 paired draws, its
condition-number-free screen covers every recorded covariance and score event
and retains only the planted five states and four edges in all 18 regimes.
Proposition 38 then composes a reusable pilot covariance with each observed
screening covariance. At 300 million screening trajectories, the resulting
candidate-specific radii average about half the fixed radius and reduce the
safe graph in every tested regime. Proposition 39 then transports this
certificate across a declared population-covariance drift. In the controlled
structure-preserving experiment, all 448 recorded events are covered, but the
screen becomes complete at maximum candidate drift `0.001628`. This separates
mathematical validity from practical selectivity.
Proposition 40 assigns a separate confidence budget to estimating that drift
from a current calibration cohort. Experiment Z shows that the resulting
screen is valid but, on this construction, less selective than treating the
same current covariance as a refreshed reference.
Proposition 41 then replaces independence across samples by a known separable
Gaussian time-correlation matrix. Its two effective sample sizes control the
quadratic and linear parts of the concentration radius, which is propagated
through every local score, transport score, and safe-graph decision. Experiment
AA verifies the resulting radius on exact stationary AR(1) draws while showing
where the i.i.d. formula ceases to cover. This is a narrow dependent-sample
theorem: its known-mean restriction is addressed by Proposition 42.
Proposition 42 removes the constant unknown mean exactly. Centering projects
the temporal covariance through \(P\), changes the unbiased denominator to
\(d_R=\operatorname{tr}(PR)\), and produces centered effective sample sizes.
Experiment AB verifies translation invariance and recorded coverage while
retaining the unresolved assumptions: the temporal envelope is known, the mean
is constant, and space-time covariance is separable. Proposition 43 estimates
a shared nonnegative AR(1) coefficient from standardized calibration channels
on the same record and carries both correlation and centering-normalization
uncertainty through the complete screen. Joint spatial-temporal estimation and
nonseparable sliding windows remain open; literal channel reuse is currently
demonstrated only for identity spatial covariance.

## Read the project

| Document | Contents |
| --- | --- |
| [Research overview](docs/research_overview.md) | One-page map of the question, 43 propositions, 29 experiments, evidence, code, and limitations |
...
| [Proved results and open problems](docs/proofs_and_conjectures.md) | Forty-three proved statements and the remaining sharpness questions |
...
construction remain open work. A separable Gaussian dependence model with an
unknown constant mean and a shared nonnegative AR(1) coefficient can now be
handled together when independent standardized calibration channels are
available. Unknown spatial whitening, time-varying means, general sliding
windows, and non-Gaussian guarantees remain open.
...
This is an ongoing study, not a finished paper. Version 0.31.0 adds a
same-record confidence interval for a shared nonnegative AR(1) coefficient,
propagates its uncertainty through mean-centered covariance and the complete
observer screen, and audits the composition in 896 stationary draws. Every proposition,
experiment, figure, command, and limitation can be followed from the central overview to its
detailed source. The repository will change as adaptive bounds, harder
